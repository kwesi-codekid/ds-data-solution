import ThemeSwitcher from "../ui/theme-switcher";
import { Button } from "@nextui-org/react";
import { Link, useNavigate } from "@remix-run/react";

const Navbar = ({ showAdminButton = true }: { showAdminButton?: boolean }) => {
  const navigate = useNavigate();

  return (
    <nav className="backdrop-blur-sm bg-indigo-700 h-16 px-4">
      <div className="max-w-5xl mx-auto flex items-center justify-between h-full">
        <Link
          to={"/"}
          className="font-poppins font-extrabold text-white text-xl"
        >
          DS Data Solutions
        </Link>

        <div className="flex items-center gap-2 md:gap-4">
          <ThemeSwitcher />

          {showAdminButton && (
            <Button
              size="sm"
              className="!text-white flex items-center justify-center md:!h-8 bg-indigo-500 !rounded-full md:rounded-xl !p-3 md:w-max"
              variant="light"
              radius="full"
              onClick={() => navigate("/admin")}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
                className="size-5"
              >
                <path d="M3.78307 2.82598L12 1L20.2169 2.82598C20.6745 2.92766 21 3.33347 21 3.80217V13.7889C21 15.795 19.9974 17.6684 18.3282 18.7812L12 23L5.6718 18.7812C4.00261 17.6684 3 15.795 3 13.7889V3.80217C3 3.33347 3.32553 2.92766 3.78307 2.82598ZM12 11C13.3807 11 14.5 9.88071 14.5 8.5C14.5 7.11929 13.3807 6 12 6C10.6193 6 9.5 7.11929 9.5 8.5C9.5 9.88071 10.6193 11 12 11ZM7.52746 16H16.4725C16.2238 13.75 14.3163 12 12 12C9.68372 12 7.77619 13.75 7.52746 16Z"></path>
              </svg>
              <p className="hidden md:block font-nunito">Admin Login</p>
            </Button>
          )}
        </div>
      </div>
    </nav>
  );
};

export default Navbar;
